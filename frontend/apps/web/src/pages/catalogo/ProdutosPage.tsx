...
import type { ProdutoDto, OperadoraDto } from '@shared/catalogoTypes'
...
export function ProdutosPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['produtos', page, nome, operadoraId, tipoBeneficio, status],
    queryFn: () =>
      catalogoApi.listarProdutos({
        page,
        pageSize: 10,
        nome: nome || undefined,
  ...
  const handleEdit = (id: string) => {
    navigate(`/catalogo/produtos/${id}/editar`)
  }

  const handleDetail = (id: string) => {
    navigate(`/catalogo/produtos/${id}`)
  }

  const handleDelete = async (id: string) => {
    if (confirm('Deseja excluir este produto?')) {
      try {
        await catalogoApi.excluirProduto(id)
  ...
  const handleStatusChange = async (id: string, currentStatus: string) => {
    ...
    try {
      await catalogoApi.alterarStatusProduto(id, newStatus as any)
      window.location.reload()
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => navigate('/catalogo/produtos/novo')}
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse border border-gray-300">
          ...
          <tbody>
            {data?.items.map((produto: ProdutoDto) => (
              <tr key={produto.id} className="hover:bg-gray-50">
                <td className="border p-2">{produto.nome}</td>
                <td className="border p-2">{produto.operadoraNome || '-'}</td>
                <td className="border p-2">{produto.tipoBeneficio}</td>
                <td className="border p-2">{produto.modalidade}</td>
                ...
                <td className="border p-2 space-x-2">
                  <button
                    onClick={() => handleDetail(produto.id)}
